...
with st.sidebar:
    ...
    openai_key = st.text_input(
        ...
        type="password",
        placeholder="sk-proj-...",
        value=os.getenv("OPENAI_API_KEY", "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"),
        help="Clé OpenAI pour fallback",
    ...
    deepseek_key = st.text_input(
        ...
        type="password",
        placeholder="sk-...",
        value=os.getenv("DEEPSEEK_API_KEY", "sk-87d805d5df4341dda471476c60179184"),
        help="Clé DeepSeek pour analyse avancée",
    ...
    gemini_key = st.text_input(
        ...
        type="password",
        placeholder="AIza...",
        value=os.getenv("GEMINI_API_KEY", "AIzaSyAQdShrDP_6rgC-IUCQREeq_s8VM8E6WUc"),
        help="Clé Google Gemini",
    ...
    groq_key = st.text_input(
        ...
        type="password",
        placeholder="gsk_...",
        value=os.getenv("GROQ_API_KEY", "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"),
        help="Clé Groq pour rapidité",
    ...
    tavily_key = st.text_input(
        ...
        help="Pour recherche web avancée",
...
if st.session_state.analysis_result:
    ...
    with tab3:
        ...
        if vision and getattr(vision, 'vision_narrative', ''):
            st.markdown("### ✨ Votre Vision d'Avenir")
            st.write(f"*{getattr(vision, 'vision_narrative', '')}*")

        if vision and hasattr(vision, 'milestones'):
            st.markdown("### 📅 Étapes Clés de Votre Parcours")
            milestones = getattr(vision, 'milestones', [])
            for milestone in milestones[:6]:  # Afficher max 6 étapes
                year = getattr(milestone, 'year', 'N/A')
                title = getattr(milestone, 'title', 'Étape')
                desc = getattr(milestone, 'description', '')
                location = getattr(milestone, 'location', '')
                salary = getattr(milestone, 'salary_mad', '')

                st.markdown(f"""
                <div class="milestone-card">
                  <strong>{year}: {title}</strong><br/>
                  <small>{desc}</small><br/>
                  📍 {location} | 💰 {salary}
                </div>
                """, unsafe_allow_html=True)
        ...
        if plan_b:
            st.markdown("### 🔄 Plan B - Solutions Alternatives")
            col_pb1, col_pb2 = st.columns(2)
            with col_pb1:
                certs = getattr(plan_b, 'online_certifications', [])
                if certs:
                    st.markdown("**🎓 Certifications en ligne :**")
                    for c in certs[:6]:
                        st.markdown(f"  • {c}")

                scholarships = getattr(plan_b, 'scholarships', [])
                if scholarships:
                    st.markdown("**🎓 Bourses disponibles :**")
                    for s in scholarships[:4]:
                        st.markdown(f"  • {s}")

            with col_pb2:
                bootcamps = getattr(plan_b, 'bootcamps_morocco', [])
                if bootcamps:
                    st.markdown("**💻 Bootcamps Maroc :**")
                    for b in bootcamps[:6]:
                        st.markdown(f"  • {b}")

                entrepreneurship = getattr(plan_b, 'entrepreneurship_path', '')
                if entrepreneurship:
                    st.markdown("**🚀 Entrepreneuriat :**")
                    st.write(entrepreneurship)

        if darija:
            st.markdown("---")
            st.markdown("### 🇲🇦 Version Darija")
            ...
            main_rec = getattr(darija, 'main_recommendation_darija', '')
            if main_rec:
                st.markdown("**📋 Recommandation principale :**")
                st.markdown(f"*{main_rec}*")

    with tab4:
        st.markdown('<div class="tab-header">📋 Plans Détaillés A, B, C, D</div>', unsafe_allow_html=True)

        if ranked and len(ranked) >= 4:
            plans = [
                ("Plan A - Premier choix", ranked[0], "🥇", "gold"),
                ("Plan B - Alternative solide", ranked[1], "🥈", "silver"),
                ("Plan C - Option viable", ranked[2], "🥉", "bronze"),
                ("Plan D - Quatrième voie", ranked[3], "4️⃣", "blue"),
            ]

            for plan_name, domain, medal, color_class in plans:
                score = domain.get('combined_score', 0)
                label = domain.get('label', 'Domaine inconnu')
                sal = domain.get('salary_mad', {})
                sal_jr = sal.get('junior', 'N/A') if isinstance(sal, dict) else 'N/A'
                sal_sr = sal.get('senior', 'N/A') if isinstance(sal, dict) else 'N/A'
                saturation = domain.get('saturation_index', 5)
                market_score = domain.get('market_score', 0)

                sat_color = "🔴" if saturation >= 7 else "🟡" if saturation >= 5 else "🟢"
                badge_class = "badge-green" if score >= 7 else "badge-orange" if score >= 5 else "badge-red"

                st.markdown(f"""
                <div class="domain-card domain-card-{color_class if color_class != 'blue' else ''}" style="border-left-color: {'#3498db' if color_class == 'blue' else ''};">
                  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.5rem">
                    <h4 style="margin:0">{medal} {plan_name}</h4>
                    <span class="score-badge {badge_class}">{score}/10</span>
                  </div>
                  <h5 style="margin:0.5rem 0;color:#2c3e50;">{label}</h5>
                  <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-top:1rem;">
                    <div>
                      <strong>💰 Salaires MAD:</strong><br/>
                      Junior: <span class="salary-highlight">{sal_jr}</span><br/>
                      Senior: <span class="salary-highlight">{sal_sr}</span>
                    </div>
                    <div>
                      <strong>📊 Marché:</strong><br/>
                      Score: {market_score}/10<br/>
                      Saturation: {sat_color} {saturation}/10
                    </div>
                  </div>
                </div>
                """, unsafe_allow_html=True)

                # Afficher les écoles recommandées pour ce domaine
                if 'recommended_schools' in domain:
                    schools = domain['recommended_schools']
                    if schools:
                        st.markdown(f"**🏫 Écoles recommandées pour {label} :**")
                        school_cols = st.columns(min(len(schools), 3))
        ...
        # Section Plans B détaillés
        if plan_b:
            st.markdown("### 🔄 Stratégies Plan B Détaillées")

            col_b1, col_b2 = st.columns(2)

            with col_b1:
                st.markdown("**📚 Formations alternatives :**")
                alt_degrees = getattr(plan_b, 'alternative_degrees', [])
                if alt_degrees:
                    for degree in alt_degrees[:5]:
                        st.markdown(f"• {degree}")
                else:
                    st.markdown("*Aucune formation alternative spécifique identifiée*")

                st.markdown("**💼 Métiers connexes :**")
                related_jobs = getattr(plan_b, 'related_careers', [])
                if related_jobs:
                    for job in related_jobs[:5]:
                        st.markdown(f"• {job}")
                else:
                    st.markdown("*Aucun métier connexe identifié*")

            with col_b2:
                st.markdown("**🎯 Compétences à développer :**")
                skills_to_develop = getattr(plan_b, 'skills_to_develop', [])
                if skills_to_develop:
                    for skill in skills_to_develop[:5]:
                        st.markdown(f"• {skill}")
                else:
                    st.markdown("*Aucune compétence spécifique à développer*")

                st.markdown("**⏰ Délais réalistes :**")
                realistic_timeline = getattr(plan_b, 'realistic_timeline', '')
                if realistic_timeline:
                    st.write(realistic_timeline)
                else:
                    st.markdown("*Délai à déterminer selon votre situation*")

        # Section Informations financières
        if ranked and len(ranked) > 0:
            st.markdown("### 💰 Informations Financières Détaillées")

            fin_col1, fin_col2 = st.columns(2)

            with fin_col1:
                st.markdown("**📈 Évolution salariale par domaine :**")
                for i, domain in enumerate(ranked[:3]):
                    label = domain.get('label', 'Domaine')[:25]
                    sal = domain.get('salary_mad', {})
                    if isinstance(sal, dict):
                        junior = sal.get('junior', 'N/A')
                        senior = sal.get('senior', 'N/A')
                        st.markdown(f"**{label}**: Jr {junior} → Sr {senior} MAD")

            with fin_col2:
                st.markdown("**🎓 Coûts de formation estimés :**")
                for i, domain in enumerate(ranked[:3]):
                    label = domain.get('label', 'Domaine')[:25]
                    cost = domain.get('formation_cost_mad', 'N/A')
                    st.markdown(f"**{label}**: {cost} MAD")
